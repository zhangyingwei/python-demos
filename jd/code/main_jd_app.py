...
def read_csv(file_type):
    ...
    login_path = os.path.join(utils.env.data_path,login_path)
    trade_path = os.path.join(utils.env.data_path,trade_path)
...
# 读文件
# login_table,trade_table = read_csv(Files.type_test)
login_table,trade_table = read_csv(Files.type_traid)
...
def formate_tables(login_table,trade_table):
    ...
    result_table.insert(10,"is_scan",is_scan)
    # is_sec
    is_sec = result_table.pop("is_sec")
    is_sec = formate_is_scan(is_sec)
    result_table.insert(11,"is_sec",is_sec)
    # all time
    ...
    result_table.insert(12,"login_time",time_col)
...
def bulid_data():
    result_table = formate_tables(login_table,trade_table)
    labels = result_table.pop("is_risk")
    return result_table,labels

def save_data(name=""):
    result_table,labels = bulid_data()
    result_table = numpy.array(result_table.values)
    labels = numpy.array(labels.values)
    with open(os.path.join(utils.env.data_path,"result_table"+name+".npy"),"wb") as result_file:
        numpy.save(result_file,result_table)

    with open(os.path.join(utils.env.data_path,"labels"+name+".npy"),"wb") as labels_file:
...
def read_data(name=""):
    with open(os.path.join(utils.env.data_path,"result_table"+name+".npy"),"rb") as result_file:
        result_table = numpy.load(result_file)
    with open(os.path.join(utils.env.data_path,"labels"+name+".npy"),"rb") as labels_file:
        labels_table = numpy.load(labels_file)
    return result_table,labels_table

# 数据 标签
result_table,labels_table = read_data()
# result_table,labels_table = read_data(name="-test")
